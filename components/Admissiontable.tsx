...
const AdmissionTableList = ({ university_raw_data, advertisementData }) => {
  ...
  async function filteredUniversitiesData(data) {
    ...
    const transformedFilteredUniversities =
      transformUniversityData(filteredUniversities);
  ...
  return (
    <div>
      <div className="HIDE FILTER BOX OR NOT mb-11">
        <h1
          className="border-b border-primary cursor-pointer inline-block select-none max-lg:text-base lg:text-xl xl:text-2xl text-primary font-bold"
      ...
      <div
        className={
          loading
            ? `block text-yellow-400 text-3xl pb-12 italic ${gotham.className}`
            : "hidden"
        }
      >
        <SkeletonUI />
      ...
      <div className="flex justify-between items-center mb-4">
        <h1
          className={`glowy dark:glowy_3 max-sm:text-2xl sm:text-4xl border-b inline-block border-rose-600 tracking-wider ${DharmaGothicRegular.className}`}
        >
      ...
      <div className="MainContaineroftable_and_Advertisement flex gap-6 flex-wrap lg:flex-nowrap max-[768px]:flex-col">
          ...
          <table className="min-w-full table-auto rounded-lg border border-primary border-collapse">
            <thead>
              <tr className="bg-primary text-secondary uppercase text-xs md:text-sm leading-normal">
                <th className="max-[350px]:text-[0.6rem] py-3 px-4 md:px-6 text-left max-md:hidden md:table-cell">
                  Sr.
                </th>
                <th className="max-[350px]:text-[0.6rem] py-3 px-4 md:px-6 text-left">
                  University
                </th>
                <th className="max-[350px]:text-[0.6rem] py-3 px-4 md:px-6 text-left">
                  Programs
                ...
                <th className="max-[350px]:text-[0.6rem] py-3 px-4 md:px-6 text-left whitespace-nowrap">
                  Deadline
                </th>
              </tr>
            </thead>
                ...
                >
                  ...
                  <td className="py-3 max-md:px-4 md:px-6 relative">
                    ...
                    <Link
                      ...
                      className="max-[340px]:text-[0.7rem] sm:text-xs block md:text-sm"
                    >
                      {university.name}
                    </Link>
                  </td>
                  <td className="py-3 max-md:px-4 md:px-6 relative">
                    ...
                    <ul className="flex flex-wrap gap-1 md:gap-1">
                      {university.programs.map((program, idx) => (
                        <li
                          key={idx}
                          className="inline w-fit text-xs md:text-sm"
                        >
                          {program}
                          {idx < university.programs.length - 1 && ","}
                        </li>
                      ))}
                    </ul>
                  </td>
                  ...
                  <td className="py-3 max-md:px-4 md:px-6 relative text-red-500 font-semibold sm:whitespace-nowrap">
                    <div className="absolute inset-y-0 left-0 w-px border border-primary" />
                    <h1 className="max-[340px]:text-[0.65rem] max-md:text-xs md:text-sm">
                      {university.deadline}
                    </h1>
                  </td>
                </tr>
        ...
        <div
          className={
            loading
              ? "hidden"
              : "max-sm:mt-6 sm:mt-8 flex flex-wrap max-[1024px]:justify-center lg:justify-start gap-3 lg:flex-col"
          }
        >
